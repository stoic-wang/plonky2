...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for MulGFp5Gate {
    ...
    fn eval_unfiltered(&self, vars: EvaluationVars<F, D>) -> Vec<F::Extension> {
        ...
        for i in 0..self.num_ops {
            ...
            for (output_limb, computed_output_limb) in
                output_limbs.into_iter().zip(computed_output_limbs)
            {
    ...
    fn eval_unfiltered_circuit(
        ...
        for i in 0..self.num_ops {
            ...
            let prod_limbs = gfp5_mul_limbwise_circuit_lifted(
                builder,
                multiplicand_0_limbs,
                multiplicand_1_limbs,
            );
            let computed_output_limbs =
                gfp5_scalar_mul_limbwise_circuit_lifted(builder, c, prod_limbs);
    ...
    for MulGFp5Generator<F, D>
{
    ...
    fn run_once(&self, witness: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        let multiplicand_0_limbs: [F; 5] = MulGFp5Gate::wires_ith_multiplicand_0(self.op_idx)
            .map(|wire| get_wire(wire))
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();
        let multiplicand_1_limbs: [F; 5] = MulGFp5Gate::wires_ith_multiplicand_1(self.op_idx)
            .map(|wire| get_wire(wire))
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();
        let output_limbs =
            MulGFp5Gate::wires_ith_output(self.op_idx).map(|wire| Target::wire(self.row, wire));
...
fn gfp5_mul_limbwise_circuit_lifted<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    a: [ExtensionTarget<D>; 5],
    b: [ExtensionTarget<D>; 5],
) -> [ExtensionTarget<D>; 5] {
    let [a0, a1, a2, a3, a4] = a;
    let [b0, b1, b2, b3, b4] = b;
...
fn gfp5_scalar_mul_limbwise_circuit_lifted<F: RichField + Extendable<D>, const D: usize>(
    builder: &mut CircuitBuilder<F, D>,
    c: ExtensionTarget<D>,
    a: [ExtensionTarget<D>; 5],
) -> [ExtensionTarget<D>; 5] {
