...
use crate::hash::merkle_tree::{
    capacity_up_to_mut, fill_digests_buf, flatten_leaves, merkle_tree_prove, MerkleCap
};
    ...
    pub fn new(mut leaves: Vec<Vec<Vec<F>>>, cap_height: usize) -> Self {
        ...
        for window in leaves.windows(2) {
            ...
            if cur_leaf_len == leaves_len {
                ...
                let (leaves_1d, leaf_size) = flatten_leaves(cur);
                ...
                let new_leaves: Vec<Vec<F>> = cap
                    ...
                    .collect();
                let (leaves_1d, leaf_size) = flatten_leaves(&new_leaves);
                cap.clear();
